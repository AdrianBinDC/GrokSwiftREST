...
let gitHubUsername = "" // TODO: fill in
let gitHubPassword = "" // TODO: fill in
...
class GitHubAPIManager {
    ...
    let clientID: String = "03d298fc9341d9c5791f"
    let clientSecret: String = "ebf7e9cca4777ea3b606ad0441650fb929b562e7"
    ...
    func doGetWithBasicAuth() -> Void {
        Alamofire.request(.GET, "https://httpbin.org/basic-auth/\(gitHubUsername)/\(gitHubPassword)")
            .authenticate(user: gitHubUsername, password: gitHubPassword)
            .responseString { response in
                if let receivedString = response.result.value {
                    print(receivedString)
                }
    ...
    func doGetWithBasicAuthCredential() -> Void {
        let credential = NSURLCredential(user: gitHubUsername, password: gitHubPassword,
